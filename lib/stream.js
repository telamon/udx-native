...
module.exports = class UDXStream extends streamx.Duplex {
  ...
  _writev (buffers, cb) {
    ...
    if (buffers.length === 1) {
      ...
      const batchHandle = binding.udx_napi_batch_begin(len)
      for (let i = 0; i < len; i++) {
        binding.udx_napi_batch_set(batchHandle, i, buffers[i])
      }

      drained = binding.udx_napi_stream_writev(this._handle, req.handle, id, batchHandle) !== 0
    }
